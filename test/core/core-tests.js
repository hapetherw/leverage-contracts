...
const proxyAuthTest = async () => {
    describe('ProxyAuth', () => {
        ...
        it('...should callExecute when auth is given to proxyAuth and StrategyExecutor set', async () => {
            ...
            const encodeData = sumInputs.interface.encodeFunctionData(
                'executeActionDirect',
                [abiEncoder.encode([ "uint", "uint" ], [ 1, 2 ])]
            )

            try {
                // await proxyAuth.callExecute(proxy.address, sumInputs.address, encodedCall[1]);
                await proxyAuth.callExecute(proxy.address, sumInputs.address, encodeData);
